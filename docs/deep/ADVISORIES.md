...
**How to close the gap**: `dev_mcp`'s `ADV-10` detector (`_adv10_ungated_destructive_transition`, one of the 10 auto-run detectors) is exactly this — a keyword-heuristic lint, not a runtime gate, not enforcement, and it says so in its own output. It scans transition labels/ids for a small, explicit, documented keyword list (`delete`, `drop`, `remove`, `destroy`, `nuke`, `wipe`, `purge`, `truncate`, `terminate`, `revoke`, `kill`) and flags any match that lacks `requires_human_authorization=True`. Run it against your own map before shipping it: false positives (an "archive" transition that mentions "removing old copies" in its label) and false negatives (a genuinely destructive transition with no alarming word in its name — "finalize", "commit", "apply" can all mean delete depending on domain) are both expected and were never going to be otherwise. This is a second pair of eyes on a map's own naming, not proof of safety — the actual decision, "does this specific transition need `requires_human_authorization=True`," is still the map author's to make, the same way `ADV-04`'s `exclusive_with` still has to be declared by hand.

---

## ADV-11 — `safe_alternatives` declaration doesn't hold up structurally

**What**: `TransitionPrimitive.safe_alternatives` is declared and unvalidated — same shape as `bridges_to` elsewhere in this codebase. Nothing checks that a named alternative actually exists as a registered transition, that it's itself less destructive (not also `requires_human_authorization=True`), or that it hasn't already been denied once itself. Three ways this goes silently wrong: a typo'd `transition_id` in `safe_alternatives` surfaces as-is in the `ESCALATE` `Outcome`'s `alternatives` list and in `slice()`, pointing nowhere; a "safe" alternative that's actually gated the same way the original destructive move was, which is a strange thing to call an alternative; or an alternative that a human already declined once before, which a fresh `ESCALATE` will still cheerfully recommend as if nothing happened.

This is a distinct question from whether the model reads or chooses to use a *validly* declared alternative. That second question is explicitly not something this library tries to answer — a model that ignores a clearly surfaced, structurally sound alternative is a model-capability problem, and there is no engine-side mechanism that fixes a model choosing badly without also meaning the engine now overrides the model's choices, which is a different feature than what `step()`/`slice()` are for. `ADV-11` only checks that what the map author declared is sound. It has nothing to say about what the model does with it.

**Why this is the default**: same shape as `ADV-04`/`ADV-10` — validating a declared relationship against the rest of the map requires walking every `safe_alternatives` list against every registered transition on every mutation, for a field whose only job is to be a name the model can read. Nothing else in this codebase does that kind of live cross-reference validation on declared relationships (`bridges_to`, `incompatible_with`, `exclusive_with` are all equally unvalidated) — adding it only for `safe_alternatives` would be an inconsistent, one-off exception rather than a considered choice.

**How to close the gap**: `dev_mcp`'s `ADV-11` detector (`_adv11_unsound_safe_alternative`) checks exactly the three failure shapes above — dangling reference, alternative itself gated, alternative already denied — against whatever `ActiveState`/`SemanticMap` your scenario builds. Same tier as `ADV-10`, same honesty: a lint aid for reviewing your own map before shipping it, not a runtime gate and not proof that the declaration is meaningful to the model reading it.
